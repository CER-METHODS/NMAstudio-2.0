#!/usr/bin/env python3
"""
Test that rob and year fields are truly optional.
Tests the complete workflow without providing rob and year values.
"""
...
async def test_optional_rob_year():
    """Test complete workflow without rob and year: upload → configure → run analysis → submit → verify"""
    ...
    async with async_playwright() as p:
        ...
        try:
            print("🚀 Starting optional rob/year test...")
            ...
            # Step 3: Fill variable selection dropdowns - SKIP rob and year (leave empty)
            print("🔍 Filling variable selection dropdowns (WITHOUT rob and year)...")
            dropdowns = await page.query_selector_all("div.Select-control")

            # Only fill studlab and treat - skip rob and year to test optional fields
            column_mappings = {
                0: "unique_id",  # studlab
                1: "treat",  # treat
                # 2: SKIP (rob - optional)
                # 3: SKIP (year - optional)
            }

            for i, dropdown in enumerate(dropdowns[:4]):  # Only first 4 dropdowns
                if i in column_mappings:
                    expected_col = column_mappings[i]
                    ...
                    print(f"⏭️  Skipping dropdown {i + 1} (rob or year - optional)")
            ...
            if len(variable_dropdowns) >= 2:
                ...
                if success:
                    print("\n🎉 TEST PASSED! Analysis works without rob/year values!")
                    print("✅ rob and year are truly optional!")
                else:
                    print("\n❌ TEST FAILED! Issues with optional rob/year handling.")
...
if __name__ == "__main__":
    print("=" * 60)
    print("OPTIONAL ROB/YEAR TEST")
    print("=" * 60)
    print("This test will:")
    print("  1. Upload psoriasis dataset")
    print("  2. Configure WITHOUT providing rob and year values")
    print("  3. Run the analysis")
    print("  4. Verify that analysis completes successfully")
    print("  5. Verify that rob/year columns exist (with NaN values)")
    print("=" * 60)
    print()

    asyncio.run(test_optional_rob_year())
