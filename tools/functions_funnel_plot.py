...
def __Tap_funnelplot(node, outcome_idx, funnel_data):
    ...
    if node:
        ...
        funnel_data = pd.read_json(
            funnel_data[outcome_idx if outcome_idx else 0],
            orient="split",
        )
    ...
    range_x = (
        [min(df.TE_adj) - 3, max(df.TE_adj) + 3]
        if not np.isnan(df.TE_adj.max())
        else None
    )
        ...
        fig.update_layout(
            ...
            annotations=[
                dict(
                    ...
                    if effect_size in ["OR", "RR"]
                    else f"{effect_size} centered at comparison-specific pooled effect",
                )
            ],
        )
        fig.add_shape(
...
def __Tap_funnelplot_normal(edge, outcome_idx, net_data, pw_data):
    """
    Edge-based funnel plot - shows funnel for a specific comparison.

    Triggered by clicking an edge (connection) in the Cytoscape network graph.
    Centers the funnel plot at the pooled effect from pairwise meta-analysis.

    Parameters:
    - edge: Selected edge data from Cytoscape (list with source/target)
    - outcome_idx: Index of the outcome (0 or 1)
    - net_data: Network data storage (raw study data)
    - pw_data: Pairwise forest plot data storage (contains pooled effects)
    """
    ...
    diamond = 0  # Initialize pooled effect center point
    ...
    if edge and len(edge) == 1:
        ...
        if df.empty:
            ...
            diamond = pw_data.loc[
                (pw_data["treat1"] == t1) & (pw_data["treat2"] == t2), "TE_diamond"
            ].iloc[0]
    ...
    max_y = (
        df[f"seTE{outcome_idx}"].max() + 0.2
        if not np.isnan(df[f"seTE{outcome_idx}"].max())
        else 0.2
    )
    range_x = (
        [min(df[f"TE{outcome_idx}"]) - 3, max(df[f"TE{outcome_idx}"]) + 3]
        if not np.isnan(df[f"TE{outcome_idx}"].max())
        else None
    )
    ...
    fig = px.scatter(
        df,
        x=f"TE{outcome_idx}",
        y=f"seTE{outcome_idx}",  # log_x=xlog,
        range_x=range_x,
        range_y=[0.01, max_y + 10],
        symbol="Comparison",
        color="Comparison",
