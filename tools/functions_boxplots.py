...
def __update_boxplot(value, edges, net_data):
    ...
    if value:
        ...
        for ind, row in df.iterrows():
            df.loc[ind, "color"] = (
                active if row.Comparison in slctd_comps else non_active
            )
            df.loc[ind, "selected"] = (
                "active" if row.Comparison in slctd_comps else "nonactive"
            )
...
def __update_scatter(value, edges, net_data):
    active, non_active = "#1B58E2", "#313539"  # '#4C5353'
    if value:
        net_data = pd.read_json(get_net_data_json(net_data), orient="split")
        try:
        ...
        fig = go.Figure(
            data=[
                go.Scatter(
                    ...
                    name=comp,
                    marker=dict(
                        color=active if comp in slctd_comps else non_active,
                        size=group["sample_size"],  # Size based on sample_size column
                        sizemode="area",
                        sizeref=2.0
                        * df["sample_size"].max()
                        / (30.0**2),  # adjust for scaling
