...
import pandas as pd
import numpy as np
from assets.storage import EMPTY_STORAGE
...
def __data_modal(
    ...
    if upload and button_id == "upload_modal_data2":
        ...
        try:
            ...
            raise ValueError(
                "Data upload failed: likely UnicodeDecodeError or MultipleTypeError, check variable characters and type"
            )
        var_dict = dict()
        var_outcomes = dict()
        if search_value_format == "iv":
            if search_value_outcome2 is None:
                ...
                studlab, treat1, treat2, rob, year, TE, seTE, n1, n2 = dataselectors[
                    2:
                ]  # first dataselector is the effect size
                var_dict = {
                    studlab: "studlab",
                    treat1: "treat1",
                    treat2: "treat2",
                ...
                var_outcomes = {
                    "effect_size1": out1_type,
                    "outcome1_direction": out1_direction,
                }
            else:
                (
                    out1_type,
                ...
                var_outcomes = {
                    "effect_size1": out1_type,
                    "outcome1_direction": out1_direction,
                    "effect_size2": out2_type,
                    "outcome2_direction": out2_direction,
                }
            ...
            if search_value_outcome1 == "continuous":
                if search_value_outcome2 is None:
                    out1_type, out1_direction = dataselectors[0:2]
                    studlab, treat1, treat2, rob, year, y1, sd1, y2, sd2, n1, n2 = (
                        dataselectors[2:]
                    )  # first dataselector is the effect size
                    var_dict = {
                        studlab: "studlab",
            ...
            if search_value_outcome1 == "binary":
                if search_value_outcome2 is None:
                    out1_type, out1_direction = dataselectors[0:2]
                    studlab, treat1, treat2, rob, year, r1, n1, r2, n2 = dataselectors[
                        2:
                    ]  # first dataselector is the effect size
                    var_dict = {
                        studlab: "studlab",
                    ...
                    (
                        ...
                        n21,
                        n22,
                    ) = dataselectors[4:]  # first dataselector is the effect size
                    var_dict = {
                        studlab: "studlab",
                        treat1: "treat1",
                        treat2: "treat2",
        ...
        if search_value_format == "iv":
            ...
            treat_list = [
                {"label": "{}".format(treat_name), "value": treat_name}
        ...
        try:
            ...
            net_data_STORAGE = {"data": data.to_json(orient="split")}
        # except:
        # net_data_STORAGE = {}
    ...
    if submit and button_id == "submit_modal_data":
        return (
            not modal_transitivity_is_open,
            not modal_data_checks_is_open,
            net_data_STORAGE,
            filename_exists,
            "",
    ...
    # else:
    #     return modal_data_is_open, modal_data_checks_is_open and (modal_data_is_open), net_data_STORAGE, filename_exists, '', False, treat_list
...
def __data_trans(
    ...
    print(f"[DEBUG __data_trans] Called with:")
    print(f"  button_id: {button_id}")
    print(f"  ctx.triggered: {ctx.triggered}")
    print(f"  upload: {upload}")
    print(f"  submit: {submit}")
    print(f"  filename2: {filename2}")
    print(f"  results_ready_STORAGE (input): {results_ready_STORAGE}")
    ...
    # Only process if this callback was actually triggered by a button click
    # not ctx.triggered means this is initial page load
    if not ctx.triggered:
        print(f"[DEBUG __data_trans] No trigger - returning current state")
        return (
            modal_data_checks_is_open,
            raw_data_STORAGE,
            net_data_STORAGE,
    ...
    if button_id == "upload_modal_data2":
        # filename_exists = True if filename is not None else False
        filename_exists = True if filename is not None else False
        ...
        if search_value_format == "iv":
            ...
            effect_size = {
                f"effect_size{i + 1}": effect_data[i] for i in range(number_outcomes)
            }
            direct = {
                f"outcome{i + 1}_direction": direct_data[i]
                for i in range(number_outcomes)
            }
            ...
            iv_data = [
                variableselectors[i:j] for i, j in zip([0] + len_ci[:-1], len_ci)
            ]
            name_n = [
                [
                    f"y1{j + 1}",
                ...
                else [f"r1{j + 1}", f"n1{j + 1}", f"r2{j + 1}", f"n2{j + 1}"]
        ...
        for i_out in range(number_outcomes):
            ...
            data_user[f"outcome{i_out + 1}_direction"] = var_outs[
                f"outcome{i_out + 1}_direction"
            ]
        ...
        if search_value_format == "iv":
            ...
            unique_values = np.unique(combined_values)
            treat_list = [
                {"label": str(treat_name), "value": str(treat_name)}
                for treat_name in sorted(unique_values)
            ]
    ...
    if button_id == "submit_modal_data":
        return (
            not modal_data_checks_is_open,
            raw_data_STORAGE,
            net_data_STORAGE,
    ...
    #     return  modal_data_checks_is_open, net_data_STORAGE, filename_exists, '', False, treat_list

    print(
        f"[DEBUG __data_trans] Returning False for results_ready_STORAGE (default fallthrough)"
    )
    print(f"  This happens when callback is triggered but no button condition matched")
    return (
        modal_data_checks_is_open,
        raw_data_STORAGE,
        net_data_STORAGE,
...
def __modal_SUBMIT_button_new(
    ...
    if "submit_modal_data.n_clicks" in triggered:
        submit_modal_data_trigger = True
    token_btn_triggered = False
    if "button-token.n_clicks" in triggered:
        token_btn_triggered = True
    load_btn_triggered = False
    if "load-project.n_clicks" in triggered:
        load_btn_triggered = True
    OUT_DATA = [
        raw_data_STORAGE,
        net_data_STORAGE,
        TEMP_consistency_data_STORAGE,
        ...
        if token_btn_triggered and filename and input_token:
            ...
            if not load_btn_triggered:
                result = [
                    [
                        (
                            ...
                            if label
                            not in [
                                "user_elements_STORAGE",
                                "user_elements_out2_STORAGE",
                            ]
                            else data
                        )
                        for data in data_each
                    ]
                    for label, data_each in EMPTY_STORAGE.items()
                ][:-2] + [""]
                ...
                if usr_token_load_ in foldername:
                    RAW_DATA_USR = [pd.read_csv(f"{path}/raw_data_STORAGE_0.csv")]
                    NET_DATA_USR = [pd.read_csv(f"{path}/net_data_STORAGE_0.csv")]
                    # DEFAULT_ELEMENTS_USR = USER_ELEMENTS_USR = get_network_new(df=NET_DATA_USR[0], i=0)
                    FOREST_DATA_USR = [
                        pd.read_csv(f"{path}/TEMP_forest_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    FOREST_DATA_PRWS_USR = [
                        pd.read_csv(f"{path}/TEMP_forest_data_prws_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    LEAGUE_TABLE_DATA_USR = [
                        pd.read_csv(
                            f"{path}/TEMP_league_table_data_STORAGE_{i}.csv",
                            index_col=0,
                        )
                        for i in range(0, 3)
                    ]
                    ...
                    for j in range(out_num):
                        ...
                        LEAGUE_TABLE_DATA_USR[j] = pd.DataFrame(
                            [
                                ...
                                for idx, row in LEAGUE_TABLE_DATA_USR[j].iterrows()
                            ],
                            columns=LEAGUE_TABLE_DATA_USR[j].columns,
                            index=LEAGUE_TABLE_DATA_USR[j].index,
                        )
                    CINEMA_NET_DATA1_USR = None
                    CONSISTENCY_DATA_USR = [
                        pd.read_csv(f"{path}/TEMP_consistency_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    NETSPLIT_DATA_USR = [
                        pd.read_csv(f"{path}/TEMP_net_split_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    NETSPLIT_DATA_ALL_USR = [
                        pd.read_csv(f"{path}/TEMP_net_split_ALL_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    RANKING_DATA_USR = [
                        pd.read_csv(f"{path}/TEMP_ranking_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]
                    FUNNEL_DATA_USR = [
                        pd.read_csv(f"{path}/TEMP_funnel_data_STORAGE_{i}.csv")
                        for i in range(out_num)
                    ]

                    USER_DATA = OrderedDict(
                        raw_data_STORAGE=RAW_DATA_USR,
                    ...
                    final_return = [
                        ...
                        for label, data_each in EMPTY_STORAGE.items()
                    ][:-2] + ["\u274c" + "Token not found: please double-check"]
