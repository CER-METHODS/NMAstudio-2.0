...
def __show_forest_plot(cell, style_pair):
    ...
    if  cell is not None and len(cell) != 0 and 'colId' in cell and cell['colId'] == "direct" and cell['value'] is not None and cell['value']!= '':
        ...
        fig = px.scatter(df, x= df['RR'], y=df["studlab"].str.pad(max(LEN_FOREST_ANNOT, df["studlab"].str.len().max()), fillchar=' '),
        ...
        ticktext_list = [' ' * 5 + '{:.2f}   {:<17}'.format(x, y) for x, y in zip(df['RR'].values, df['CI'].values)]
        ...
        fig = px.scatter(df, x= df['RR'], y=df["studlab"],
                       error_x_minus=None,
                       error_x= None,
                       log_x=False,
                       size_max=10,
