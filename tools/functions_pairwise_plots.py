...
def __update_forest_pairwise(
    ...
    slctd_comps = []
    slctd_compsinv = []
    outcome_idx = 0 if not outcome_idx else int(outcome_idx)

    # Guard: Check if forest_data_prws exists and has elements
    if not forest_data_prws or len(forest_data_prws) == 0:
        fig = go.Figure()
        fig.update_layout(
    ...
    # Guard: Check if outcome_idx is within bounds
    if outcome_idx < 0 or outcome_idx >= len(forest_data_prws):
        outcome_idx = 0
    ...
    if edge:
        ...
        try:
            ...
            print(f"[ERROR] Failed to parse pairwise forest data: {e}")
            fig = go.Figure()
            fig.update_layout(
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
        ...
        # Try to get outcome direction with error handling
        try:
            net_data = pd.read_json(
                net_storage[0]
                if isinstance(net_storage, list)
                else get_net_data_json(net_storage),
                orient="split",
            )
            outcome_col = f"outcome{outcome_idx + 1}_direction"
            ...
            print(f"[ERROR] Failed to get outcome direction in pairwise: {e}")
            outcome_direction = False
        df["Comparison"] = df["treat1"] + " vs " + df["treat2"]
        # df = df[df.Comparison.isin(slctd_comps)]
        df = df[
            df["Comparison"].isin(slctd_comps) | df["Comparison"].isin(slctd_compsinv)
        ]
        if is_numeric_dtype(df["studlab"]):
        ...
        CI_lower_diamond, CI_upper_diamond = (
            df["CI_lower_diamond"].map("{:,.2f}".format),
            df["CI_upper_diamond"].map("{:,.2f}".format),
        )
        CI_d = (
            "("
            + CI_lower_diamond.astype(str)
            + ", "
    ...
    fig.update_traces(
        ...
        error_x=dict(thickness=2, color="#313539"),
    )  # '#ef563b' nice orange trace
    #
    if edge:
        fig.update_layout(
            ...
            annotations=[
                ...
                dict(
                    x=np.floor(np.log10(min(low_rng, 0.1))) if xlog else low_rng_,
                    ax=0,
                    y=0,
                    ay=-0.1,
                    yanchor="bottom",
                ...
                dict(
                    x=np.floor(np.log10(max([up_rng, 10]))) if xlog else up_rng_,
                    ax=0,
                    y=0,
                    ay=-0.1,
                    yanchor="bottom",
                ...
                dict(
                    ...
                    else f"Favours {df.treat2.iloc[0]}",
                    showarrow=False,
                ),
                dict(
                    x=np.floor(np.log10(max([up_rng, 10]))) / 2
                    if xlog
                    else df.CI_upper.max() / 2,
                    y=0,
                    xref="x",
                    yref="paper",
                    xanchor="auto",
                    ...
                    if outcome_direction
                    else f"Favours {df.treat1.iloc[0]}",
                    showarrow=False,
                ),
                dict(
                    x=0,
        ...
        fig.add_trace(
            go.Scatter(
                x=romb(
                    center, low=CI_lower_diamond.iloc[0], up=CI_upper_diamond.iloc[0]
                )["x"],
                y=romb(
                    center, low=CI_lower_diamond.iloc[0], up=CI_upper_diamond.iloc[0]
                )["y"],
        ...
        fig.update_yaxes(
            range=[-0.3, 1 + df.studlab.shape[0]],
            autorange=True,
            ticks="outside",
            tickwidth=2,
            tickcolor="black",
