...
def __generate_xlsx_league(n_clicks, leaguedata):
    # Guard: Check if leaguedata is valid
    if not leaguedata:
        return None

    # Handle case where leaguedata is a list (Div contains DataTable as child)
    if isinstance(leaguedata, list):
        # Find the DataTable in the list
        for item in leaguedata:
            if (
                isinstance(item, dict)
                and "props" in item
                and "data" in item.get("props", {})
            ):
                leaguedata = item
                break
        else:
    ...
    # Handle case where leaguedata is the DataTable component dict
    if not isinstance(leaguedata, dict) or "props" not in leaguedata:
        return None

    if "data" not in leaguedata["props"] or not leaguedata["props"]["data"]:
        return None

    df = pd.DataFrame(leaguedata["props"]["data"])
    style_data_conditional = leaguedata["props"].get("style_data_conditional", [])
    ...
    def to_xlsx(bytes_io):
        ...
        df.to_excel(
            writer, sheet_name="League_Table"
        )  # Convert the dataframe to an XlsxWriter Excel object.
        workbook, worksheet = (
            writer.book,
            writer.sheets["League_Table"],
        ...
        for r, rl in enumerate(df.columns):
            for c, cl in enumerate(df.columns):
                worksheet.write(
                    r + 1, c + 1, df.loc[rl, cl], wrap_format
    ...
    ######### return dash.dcc.send_data_frame(writer.close(), filename="league_table.xlsx")
...
def __generate_csv_consistency(n_nlicks, outcome_idx, consistencydata_all):
    ...
    if "btn-netsplit-all.n_clicks" in triggered:
        button_trigger = True

    # Check if data exists
    if consistencydata_all is None or len(consistencydata_all) == 0:
        return None
    if (
        outcome_idx >= len(consistencydata_all)
        or consistencydata_all[outcome_idx] is None
    ):
        return None
    ...
    if button_trigger:
        if df is not None:
            ...
            df = df[["Comparison", "k", "direct", "nma", "indirect", "p-value"]].round(
                decimals=4
            )
            ...
            return dash.dcc.send_data_frame(
                df.to_csv, filename="consistency_table_full.csv"
            )
...
def __generate_xlsx_netsplit(n_nlicks, consistencydata):
    ...
    def to_xlsx(bytes_io):
        ...
        df.to_excel(
            writer, sheet_name="Netsplit_Table", index=False
        )  # Convert the dataframe to an XlsxWriter Excel object.
        workbook, worksheet = (
            writer.book,
            writer.sheets["Netsplit_Table"],
        )  # Get the xlsxwriter workbook and worksheet objects.
        wrap_format = workbook.add_format({"text_wrap": True, "border": 1})
        wrap_format.set_align("center")
        wrap_format.set_align("vcenter")
        ...
        worksheet.conditional_format(
            ...
            options={
                "type": "cell",
                "format": workbook.add_format(
                    {"bg_color": "white", "font_color": "blue", "text_wrap": True}
                ),
                "criteria": "between",
                "minimum": 0.10001,
                "maximum": 0.15,
        ...
        worksheet.conditional_format(
            ...
            options={
                "type": "cell",
                "format": workbook.add_format(
                    {"bg_color": "white", "font_color": "orange", "text_wrap": True}
                ),
                "criteria": "between",
                "minimum": 0.05001,
                "maximum": 0.10,
        ...
        worksheet.conditional_format(
            ...
            options={
                "type": "cell",
                "format": workbook.add_format(
                    {"bg_color": "white", "font_color": "red", "text_wrap": True}
                ),
                "criteria": "<=",
                "value": 0.05,
            },
    ...
    ###########return dash.dcc.send_data_frame(writer.close(), filename="Netsplit_Table_table.xlsx")
